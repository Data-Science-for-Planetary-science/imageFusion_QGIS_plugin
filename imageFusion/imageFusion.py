...
/***************************************************************************
 ...
 Generated by Plugin Builder: http://g-sherman.github.io/Qgis-Plugin-Builder/
        ...
        git sha              : $Format:%H$
        copyright            : ################
        Algorithm code       : Vipul Chhabra
        email                : uday.rage@gmail.com
 ***************************************************************************/
...
from .ImageFusion.dataWriter import DataWriter
from .ImageFusion.HISTIF import HISTIF
from .ImageFusion.IHISTIF import IHISTIF
from .ImageFusion.PHISTIF import PHISTIF
...
class ImageFusion:
    ...
    def __init__(self, iface): 
        ...
        self.algoOptions = ['Standard HISTIF','Improved HISTIF','Parallel HISTIF']
    ...
    # Get raster layer names into Combo box
    ...
    # set output file name : open file dialog to save output file
    ...
    def getInputFile(self):
        self.inputFileName = QFileDialog.getSaveFileName(self.dlg, 'Save File As')
        self.dlg.lineEdit.setText(str(self.inputFileName[0]))
    
    # convert raster file into numpy array
    ...
    # convert numpy arrray into tiff file 
    ...
    def HISTIF(self):
        ...
        # 
        if version == "Parallel HISTIF": 
            print("Runnning Parallel Algorithm")
            print(coarset0.shape, coarset1.shape, finet0.shape)
            parallelFusion = PHISTIF(coarset0,coarset1,finet0)
            parallelFusion.set_parameters(parameters=params)
            params, image = parallelFusion.fusion(iterations=iterations,neighbours=neighbours)
            
        elif version == 'Standard HISTIF':
            HISTIF_Fusion = HISTIF(coarse_image_t0=coarset0,coarse_image_t1=coarset1,fine_image_t0=finet0)
            HISTIF_Fusion.set_parameters(params)
            params, image = HISTIF_Fusion.fusion(iterations=iterations, neighbours=neighbours) 
               
                
        elif version == 'Improved HISTIF':
            IHISTIF_Fusion = IHISTIF(coarse_image_t0=coarset0,coarse_image_t1=coarset1,fine_image_t0=finet0)
            IHISTIF_Fusion.set_parameters(params)
            params, image = IHISTIF_Fusion.fusion(iterations=iterations, neighbours=neighbours)       
        ...
        standard_writer = DataWriter(standard_predicted, outputFile).store_geotiff_file()
        ...
        #     print("image Fusion Completed",len(image[0]),final_image.shape)
        # final_image = final_image.T
            # tiff.imwrite(outputFile, image, bigtiff=True, photometric='rgb', planarconfig='separate', predictor=True, metadata={'axes': 'TZCYX'})
            
            # if version == 'Improved HISTIF':
            #     old_version = False
            # results = main_psf(coarse_img_t0=coarset0,coarse_img_t1=coarset1,fine_img_t0=finet0,params=params,iter=iterations,neighbors=neighbours,old_version=old_version)
        # print("--------------------------------")
        # print(results.shape)
        # results = np.reshape(results,(results.shape[0],results.shape[1],results.shape[2]))
        
        # print('result',results.shape)
        # self.tifConvert(final_image,outputFile)
    ...
    def setRecommendedValues(self):
        ...
        if status:
            #set recommended  params
            self.dlg.scaleFactor.setText(str(1))
            self.dlg.iterations.setText(str(2))
    ...
    def run(self):
        ...
        #save evaluated file 
        self.dlg.pushButton.clicked.connect(self.getInputFile)

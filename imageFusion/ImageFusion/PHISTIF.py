...
class PHISTIF:
    ...
    def process_band_fusion(self, opt_params, band_vals, band_num, optimizer_params):
        """
        ...
        pred_fine_t1 = np.multiply(np.divide(coarse_t1_psf, coarse_t0_psf), kwargs["fine_image"])      
    ...
    def fusion(self, iterations, neighbours = 4, evaluation = False, variant = 'new', optimizer = "default", n_components = 10):
        ...
        num_bands = min(self.coarse_image_t0.shape[2], self.fine_image_t0.shape[2])
        
        band_order = itertools.cycle(range(num_bands))
        ...
        with Pool(n_cores) as pool:
            ...
            for i in range(num_bands):
                ...
                args_p.append((opt_params, img_bands, band_num, optimizer_params))
...
if __name__ == "__main__":
    ...
    fusion_test = PHISTIF("Tests/test-1/coarse_t0.tif", "Tests/test-1/coarse_t1.tif", "Tests/test-1/fine_t0.tif", file_type = "tiff")
    ...
    standard_writer = DataWriter(standard_predicted, f"Predictions/PHISTIF_pred.tiff").store_geotiff_file()